...
# This installs the dependencies first if they are missing
try: 
...
def generate_population(min_v=0.001, max_v=10, size=(25), shift=0, gen_nr=0):
    # Create dictionary for easy panda conversion
    dict = {}
    ...
    # Generate dictionary
    for i in range(size):
        dict[f'nr_{i+shift}'] = {'gen': gen_nr, 'm': m[i], 'a': a[i], 'b': b[i], 'n1': n1[i], 'n2': n2[i], 'n3': n3[i]}
    
    # Convert to DataFrame for easy access later
    df = pd.DataFrame(dict).T
#    print(f"Population is of shape {len(dict)}")
...
def crossover_mutate(parents: pd.DataFrame, crossover=True, mutate=True, pop=25):
    """"
    inputs:
        parents[nr_parents, bitstring]

    Produces 2 offspring in 2 parents

    outputs:
        offspring[nr_parents/2, 2, genes]
    """
    ...
    if len(parents) > (pop // 2) == True:
        ...
        # We're not saving the parents for diversity and since we have a save function
        if crossover == True:
                ...
                try:
                    ...
                    if i % 2 == 0:
                        ...
                        if prob % 2 == 0:
                            ...
                            new_gen.loc[f'nr_{i+len(parents)}']['n1':'n3'] = cache_new_gen.loc[f'nr_{i+1}']['n1':'n3'] 
                    else:
                        if prob % 2 == 0:
                            new_gen.loc[f'nr_{i+len(parents)}']['m':'b'] = cache_new_gen.loc[f'nr_{i-1}']['m':'b']
                        else:
                    ...
                    if len(parents) % 2 != 0 and i == len(parents) - 1:
                        if prob % 2 == 0:
                            ...
                            new_gen.loc[f'nr_{i+1+len(parents)}']['n1':'n3'] = cache_new_gen.loc[f'nr_{i}']['n1':'n3']
...
def run(from_file=False):
    ...
    if from_file:
        ...
        if len(objs) > 0:
            ...
            survivors = crossover_mutate(parents, crossover=False, mutate=True)
